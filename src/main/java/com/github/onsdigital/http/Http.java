...
public class Http implements AutoCloseable {
     ...
     * @throws IOException If an error occurs.
     */
    public <T> Response<T> post(Endpoint endpoint, Document requestMessage, Class<T> responseClass, NameValuePair... headers) throws IOException {
        if (requestMessage == null) {
            return post(endpoint, responseClass, headers);
        } // deal with null case
     ...
     * @see MultipartEntityBuilder
     */
    private <T> Response<T> post(Endpoint endpoint, File file, Class<T> responseClass) throws IOException {
        if (file == null) {
     ...
     * @see MultipartEntityBuilder
     */
    public <T> Response<T> post(Endpoint endpoint, Path file, Class<T> responseClass) throws IOException {
        return post(endpoint, file.toFile(), responseClass);
    }
     ...
     * @see MultipartEntityBuilder
     */
    public Response<String> post(Endpoint endpoint, Path file) throws IOException {
        return post(endpoint, file.toFile(), String.class);
    }
     ...
     * @throws UnsupportedEncodingException If a serialisation error occurs.
     */
    private StringEntity serialiseRequestMessage(Document requestMessage) throws UnsupportedEncodingException {
        StringEntity result = null;

        // Add the request message if there is one
        if (requestMessage != null) {
            // Send the message
            String message = Xml.toString(requestMessage);
            result = new StringEntity(message);
        }
